...
do
{
	...
	switch ($action) {
		...
		case 'AddSchedule':
			...
			$queryInsert=$pdo->prepare('INSERT INTO schedule(sid,time,content,isdone,cate) VALUES (:sid,:time,:content,:isdone,:cate)');
			...
			$queryInsert->bindParam(':cate',$cate,PDO::PARAM_INT);
			...
			if($resultInsert)
			{
				...
				$ApiResult['ScheduleInfo']=array('id'=>$newid,'sid'=>$sid,'time'=>$time,'content'=>$content,'cate'=>$cate,'isdone'=>$isdone);
				break;
			}
			else
			{
		...
		case 'UpdateContent':
			...
			$queryUpdate=$pdo->prepare('UPDATE schedule SET content=:content,cate=:cate,time=:time WHERE id=:id');
			...
			$queryUpdate->bindParam(':time',$time,PDO::PARAM_STR);
