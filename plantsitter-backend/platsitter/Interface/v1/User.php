...
do
{
	...
	switch ($action) {
		...
		case 'GetCateInfo':
			...
			$queryFind=$pdo->prepare('SELECT * FROM accesstoken WHERE sid=:sid && access_token=:access_token');
			$queryFind->bindParam(':sid',$sid,PDO::PARAM_STR);
			$queryFind->bindParam(':access_token',$access_token,PDO::PARAM_STR);
			$result=$queryFind->execute();
			if($result)
			{
				$token=$queryFind->fetch();
				if($token)
				{

					$querySelect=$pdo->prepare('SELECT * FROM user WHERE sid=:sid');
					$querySelect->bindParam(':sid',$sid,PDO::PARAM_INT);
					$resultSelect=$querySelect->execute();
					if($resultSelect)
					{
						...
						if($user)
						{
							...
							$ApiResult['error_code']=0;
							$ApiResult['error_message']='';
							$ApiResult['Cate_Info']=$user['cate_info'];
							break;
						}
					}
					else
		...
		case 'UpdateCateInfo':
			...
			if($sid=='' || $access_token =='' || $cate_info=='')
			{
				$ApiResult['isSuccessed']=false;
				$ApiResult['error_code']=API_ERROR_PARM_LACK;
				$ApiResult['error_message']='lack sid and access_token';
				break;
			}
			...
			if($result)
			{
				...
				if($token)
				{
					...
					$querySelect=$pdo->prepare('UPDATE user SET cate_info=:cate_info WHERE sid=:sid');
					$querySelect->bindParam(':cate_info',$cate_info,PDO::PARAM_STR);
					$querySelect->bindParam(':sid',$sid,PDO::PARAM_INT);
					$resultUPDATE=$querySelect->execute();
					if($resultUPDATE)
					{
						$ApiResult['isSuccessed']=true;
						$ApiResult['error_code']=0;
